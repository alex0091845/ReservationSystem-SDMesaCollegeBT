...
export async function deleteEvent(eventData) {
    try {
        return await request(`/events/${eventData.id}`, "DELETE");
    } catch (error) {
        console.error("Could not delete reservation on backend:", error);
        throw new Error(
            getRequestFailureMessage(error, "Could not delete the reservation.")
        );
    }
}
